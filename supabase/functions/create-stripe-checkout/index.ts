  ...
  try {
    ...
    const stripeResponse = await fetch('https://api.stripe.com/v1/checkout/sessions', {
      ...
      body: new URLSearchParams({
        ...
        'success_url': `${Deno.env.get('SUPABASE_URL')}/functions/v1/stripe-success?session_id={CHECKOUT_SESSION_ID}&signup_id=${signup_id}`,
        'cancel_url': `${req.headers.get('origin') || 'https://dmcgxjmkvqfyvsfsiexe.supabase.co'}/`,
    ...
    console.error('Erreur create-stripe-checkout:', error);
    return new Response(JSON.stringify({ 
      error: error.message,
      success: false 
    }), {
